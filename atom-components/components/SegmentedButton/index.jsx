 ...
 *
 * variant="text"（默认）：文本类按钮，默认总宽 514px，可通过 width 调整，子按钮按数量等分自适应；
 * variant="icon"：纯图标类按钮，高 32px、轨道 padding 2px、每段宽 36px、图标 14px。
 * 文本类按钮通过 showIcon 开关图标、count 控制子按钮数量、labels 自定义文本。
 *
 * 状态覆盖：默认、悬停、按压、禁用（整组或单段）。
 */
...
export default function SegmentedButton({
  ...
  const rootClass = [
    ...
    variant === "icon" || radius === 8 ? "sb-radius-8" : "",
  ...
  return (
    <div className={rootClass} role="group" aria-label="SegmentedButton" style={width === undefined ? undefined : { width }}>
      {resolvedSegments.map((seg, idx) => {
        ...
        return (
          <button
            ...
            aria-label={variant === "icon" ? (seg.ariaLabel ?? seg.label ?? labels?.[idx] ?? `选项 ${idx + 1}`) : undefined}
          ...
          >
            ...
            {variant !== "icon" && seg.label ? <span className="sb-label">{seg.label}</span> : null}
          </button>
        );
      })}
    </div>
