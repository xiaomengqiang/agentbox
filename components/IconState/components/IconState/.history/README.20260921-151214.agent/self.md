...
图标状态组件：一个可点击的图标按钮，内置 默认 / 悬浮 / 激活 / 禁用 四种状态。外围 32px、图标 20px，默认图标为 90% 黑色的搜索图标；`icon` 变量可自定义填写任意图标名（内置 `search` / `menu` 两个示例）。
...
- `icon` 变量可自定义填写任意图标名（默认 `search`），也可用 `src` 直接传入自定义图标
...
| `icon` | `string` | `"search"` | 图标变量名（可自定义填写），需在 `ICON_SRC` 中配置对应 SVG 路径 |
...
通过 `icon` prop 传入图标名即可切换，名字可自定义（不限于内置的 `search` / `menu`）：

```jsx
<IconState icon="search" /> {/* 搜索图标（默认） */}
<IconState icon="menu" />   {/* 菜单图标 */}
<IconState icon="my-icon" /> {/* 任意自定义图标名 */}
```

`icon` 与 SVG 资源的对应关系定义在 `index.jsx` 顶部的 `ICON_SRC` 映射中：
...
**新增图标**：把新 SVG 放入 `assets/uploads/`，再在 `ICON_SRC` 中追加一行 `图标名: "./assets/uploads/新图标.svg"`，即可用 `<IconState icon="图标名" />` 使用。
