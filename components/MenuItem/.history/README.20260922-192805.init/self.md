...
### 下拉列表（dropdown）
- 单行菜单项 152×32px、6px 圆角，图标 + 文本左对齐
...
- `dropdown`：`{ id, label, icon?, iconSrc?, danger?, disabled?, state? }`；`icon` 为 Lucide 图标名（`message-square-plus` 新建对话 / `trash-2` 卸载），`iconSrc` 为自定义上传图片路径；`danger` 标记危险项（红字），`disabled` 标记禁用（50% 透明度），`state` 可选 `hover`
